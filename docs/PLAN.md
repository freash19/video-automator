...
## Pending Tasks
- [x] Подчинить вставку биролов. (Исправлено: улучшены селекторы источников/ориентации, переработан механизм поиска и активации сцен).
- []  Подчинить запуск задач, т.к. на текущий момент после нажатия кнопки ЗАПУСТИТЬ не запускаются браузеры и крашится хром. Хотя через кнопку Открыть браузер они запускаются. Привести в рабочий режим и протестировать на одном из загруженных сценариев, добавив 3 сцены из цсв файла (проверку проведи в profile_3, там авторизован аккаунт). В конечном итоги должны запускаться разные профили при выборе, не крашится, работать скрипт с хайгеном. 
